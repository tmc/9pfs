...
func (s *Server) impVolumeReclaim(self objc.ID, _ objc.SEL, item, reply objc.ID) {
	if volume, ok := s.volumeFor(self); ok {
		if it, ok := s.itemFor(item); ok {
			...
			// Reclaim retires this FSItem, so drop both directions of the
			// mapping. Leaving the Item -> FSItem entry would hand a reclaimed
			// object back to FSKit on the next lookup of the same file.
			s.objs.Delete(it)
			s.items.Delete(item)
		}
	}
